...
| Escolhas explícitas encadeadas | HTTPie e `python-requests-toolbelt` conservam a classificação explícita após recriação e restauração; remover HTTPie recursivamente preserva a biblioteca selecionada separadamente. |
...
Builds locais do checkpoint inicial T08: base `sha256:d14aec63996ff2d0d43d899b854d0a357464a8966684f1d6c99d59c3a8fb808a` e Salesforce `sha256:dc153d561be8989a661572b57da5ea7763708ae79f28572753d71e04a088e15e`. Os rótulos de revisão desses builds de desenvolvimento não são recibos de release: a base reutiliza rótulos T02 para cache; Salesforce aponta ao merge T07. O CI constrói suas imagens com o SHA da PR.

O export dos comandos passou em gofmt, go vet e builds Windows/Linux. A sintaxe Python, ShellCheck, shfmt dos novos scripts e `git diff --check` passaram. A configuração de workflow foi analisada por yq. A aceitação Linux/CI será registrada após a publicação da PR; o ciclo de revisão independente está em [T08: revisão independente](t08-review.md).

Antes do checkpoint de revisão, passaram os oito casos de pacotes em 367,297 s, comandos Windows (3, 89,083 s), preparo (3, 138,116 s), projetos/troca (2, 35,837 s) e Compose (1, 52,468 s); os tempos incluem o runner. O teste de falha local ganhou depois uma verificação de renderização real do Chrome, validada junto da correção da revisão. Os catorze blocos Bash do workflow também passaram em `bash -n`.

A primeira passagem Salesforce falhou em DNS durante o reparo de extensões (`ENOTFOUND marketplace.visualstudio.com`) e a resolução inicial do Chrome (`Name or service not known`), em 340,048 s. As sondas posteriores fizeram quarenta consultas em cada ambiente Windows, base e Salesforce, sem falhas, e não estabeleceram a causa original. Nenhuma configuração de DNS ou TLS foi alterada. A primeira saída foi preservada como evidência extraída do processo; o runner de desenvolvimento foi ajustado para não sobrescrever logs em tentativas futuras.
...
Na repetição, Salesforce passou (2 testes, 498,933 s) e conectividade passou (4, 131,248 s). O teste de rede dos aplicativos falhou em 198,770 s com interrupção TLS no Marketplace durante a instalação do Pack no Insiders, após concluir os aplicativos, a CLI e o Pack no Stable. A fixture foi preservada e a retomada passou em 78,918 s incluindo o runner, sem alteração de TLS. Isso comprova retomada; uma instalação nova sem essa interrupção continua sendo uma verificação distinta no CI.

Depois da revisão independente, o encadeamento real de dependências revelou perda da classificação explícita. A correção e a prova adicional de renderização do Chrome passaram juntas em 154,828 s. As imagens reconstruídas são base `sha256:838eca2dbdaae2c1ef20aa367bd18a260195c34f49718b0782bb1627543dae86` e Salesforce `sha256:403577894a32c9279df967e56fbde4cbeda4cb71b49928098d25ebe898caa910`. As tabelas de doze pacotes da instalação Salesforce coincidem nos dois logs de build; o ID antigo já não estava disponível para comparar diretamente o inventário completo. Os sete cenários restantes de pacotes passaram em 369,493 s, completando os nove casos nas imagens corrigidas em dois grupos sem sobreposição.
