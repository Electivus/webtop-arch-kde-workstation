...
A retomada em 2026-09-16 usa Docker Desktop com WSL2, conforme DEC-037/DEC-038. Os dois cenários das correções passaram também pelo controlador Linux: recuperação do perfil legado em 29,396 s e interrupção do comando em 41,241 s. A preparação, os problemas encontrados na regressão e seus resultados finais estão em [Retomada WSL2](wsl2-resume.md).

Depois do acompanhamento independente, a execução completa identificou um diagnóstico incorreto para volume alheio, uma fixture que não estabelecia a identidade da imagem, a confirmação interativa do lançador oficial do VS Code em WSL2 e um prazo insuficiente do teste de cópias grandes. Foram corrigidos e revalidados até `66f705e8ffca613bb5d370d11318f4c070e15b8e`, sem reabrir o ciclo interno de revisão e sem remover asserções. A análise desses ajustes foi feita pelo implementador; não é uma nova revisão independente. A regressão Windows encerrou com sucesso, incluindo atualização, falhas e recuperação dos aplicativos e das duas variantes.
